...
def start_game(update: Update, context: CallbackContext):
    ...
    if game.host:
        ...
        return
    # İstifadəçinin aparıcı sayını artırın
    update_scores(user_id, update.effective_user.first_name, chat_id, update.effective_chat.title, host_count_increment=1)
    keyboard = [
        [
    ...
    context.bot.send_message(chat_id, "🎮 Oyun başlamağa hazırdır. Başlamaq istəyirsiniz?", reply_markup=reply_markup)
...
def check_answer(update: Update, context: CallbackContext):
    ...
    game = games.get(chat_id)
    if not game or not game.host or not game.is_active or not answer:
        return
    ...
    if answer == correct_word:
        try:
            # İstifadəçinin doğru cavab sayını və xalını artırın
            update_scores(user_id, update.effective_user.first_name, chat_id, update.effective_chat.title, correct_answers_increment=1, score_increment=10)
            if game.mode == "full":
                game.set_host(user_id, update.effective_user.username)
            else:
...
def button_callback(update: Update, context: CallbackContext):
    ...
    if query.data == 'start_full_game':
        if game.host:
            context.bot.answer_callback_query(query.id, text="⚠️ Oyunda artıq aparıcı var!", show_alert=True)
            return
        ...
        if not game.host:
            ...
            context.bot.answer_callback_query(query.id, text="⚠️ Artıq aparıcı var!", show_alert=True)
        ...
        if game.host and game.host['id'] == user_id:
            ...
            context.bot.answer_callback_query(query.id, text="⚠️ Siz aparıcı deyilsiniz!", show_alert=True)
    elif query.data == 'show_word':
        if game.host and game.host['id'] == user_id:
            context.bot.answer_callback_query(query.id, text=f"🎯 Sizin sözünüz: {game.current_word}", show_alert=True)
        else:
            context.bot.answer_callback_query(query.id, text="⚠️ Siz aparıcı deyilsiniz!", show_alert=True)
        ...
        if game.host and game.host['id'] == user_id:
            ...
            while game.current_word == old_word:
                game.current_word = random.choice(words)
            context.bot.answer_callback_query(query.id, text=f"🎲 Yeni sözünüz: {game.current_word}", show_alert=True)
        else:
            context.bot.answer_callback_query(query.id, text="⚠️ Siz aparıcı deyilsiniz!", show_alert=True)
    elif query.data == 'do_nothing':
        context.bot.answer_callback_query(query.id, text="⚠️ Hələlik heç bir şey edilmədi.", show_alert=True)
